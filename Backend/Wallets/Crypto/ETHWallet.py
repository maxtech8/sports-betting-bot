...
from Wallets.Configuration.Keys import infura_id
from Wallets.WalletTypes import WalletTypes

logger = CustomLogger.get_instance()


class ETHWallet(AbstractCryptoWallet):
    def __init__(self, user_id):
        # Connect to Ethereum Mainnet
        super().__init__("ETH", "Ethereum", user_id)
        self.eth_mainnet = f"https://mainnet.infura.io/v3/{infura_id}"  # Replace with your Infura Project ID
        self.web3 = Web3(Web3.HTTPProvider(self.eth_mainnet))
        print("Connected to ETH:", self.web3.is_connected())

    async def get_crypto_balance(self):
        address = await self.get_address()
        print(f"{self.user_id} - ETH - public_key={self._address}")
        print(f"{self.user_id} - ETH - private_key={self._private_key}")
        balance = self.web3.eth.get_balance(address)
        human_readable_balance = self.web3.from_wei(balance, 'ether')
        float_balance = float(human_readable_balance)
        print(f"{self.user_id} Balance:", float_balance, "ETH")
